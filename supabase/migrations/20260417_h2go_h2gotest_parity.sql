...
ALTER TABLE public.auth_sessions
  ADD COLUMN IF NOT EXISTS user_id TEXT REFERENCES public.app_users(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS app_users_registration_idx
  ON public.app_users (is_registered, is_active);
...
CREATE INDEX IF NOT EXISTS alerts_device_ts_idx
  ON public.alerts (device_id, ts DESC);

CREATE INDEX IF NOT EXISTS alerts_device_resolved_ts_idx
  ON public.alerts (device_id, is_resolved, ts DESC);

CREATE INDEX IF NOT EXISTS alerts_resolved_ts_idx
  ON public.alerts (is_resolved, ts DESC);
...
INSERT INTO public.devices (id, name, location, status, is_on, last_seen)
